...
def in_any_attach(searchedText, listAttachs, regexp=True):
   if regexp:
      for attach in listAttachs:
         ...
         if 'text' in attach[type_attach] \
            and re.search(searchedText, attach[type_attach]['text'], re.I):
               return True;
   else:
      for attach in listAttachs:
         type_attach = attach['type'];
         if 'text' in attach[type_attach] \
            and searchedText in attach[type_attach]['text'].lower():
               return True;
   return False;
...
class VKStuffApp(QtWidgets.QMainWindow, design.Ui_MainWindow):
   def __init__(self, app):
      ...
      self.vkps_getPostButton.clicked.connect(self.vkps_get_post)  # Устаревшая кнопка, ничего не делает
      ...
      try:
         with open('vk_config.v2.json', 'r') as f:
            js = json.load(f)
         self.logins = {login:'' for login in js};
  ...
  ### Сигналы
   ...
   def vkspp_htmlGen(self):
      ...
      with open('posts.html', 'wt', encoding='utf-8') as html:
         ...
         html.write('<body>\n');
         url_pattern = re.compile(r"([^.\s]+\.{1}[^./\s]{2, 6}(?:/[^\s]*)*)");
         http_pattern = re.compile(r'(href=(?!http).*?)');
         for post in post_list:
            ...
            text = url_pattern.sub(r'<a href=\1>\1</a>', post["text"])
            text = http_pattern.sub(r'\1http://', text)
            text = text.replace("\n", "<br>\n");
            ...
            if "attachments" in post:
               for attachment in post["attachments"]:
                  if "photo" in attachment:
                     ...
                     img = attachment["photo"]["sizes"][3]["url"];
...
#
   def vked_get_post(self):
      if ('vk.com' in self.vked_postLink.text() and 'wall' in self.vked_postLink.text()) \
       or 'vk.com' not in self.vked_postLink.text():
         ...
         if post:
            ...
            if owner_id > 0:
               name = f"{name}" \
                      f"{self.vk.method('users.get', {'user_ids': owner_id})[0]['first_name']} " \
                      f"{self.vk.method('users.get', {'user_ids': owner_id})[0]['last_name']}"
            else:
               name = f"{name}" \
                      f"{self.vk.method('groups.getById', {'group_id': -owner_id})[0]['name']}"
   ...
   def vked_publishPost(self):
      ...
      for row in range(0, rowCount):
         if     self.vked_attachments.item(row, 0) \
            and self.vked_attachments.item(row, 0).text() \
            and self.vked_attachments.item(row, 1) \
            and self.vked_attachments.item(row, 1).text():
               if self.vked_attachments.item(row, 0).text() == 'link':
                  attachments = f"{attachments}" \
                                f"{self.vked_attachments.item(row, 1).text().replace(', ', '')},"
               else:
                  attachments = f"{attachments}" \
                                f"{self.vked_attachments.item(row, 0).text()}" \
                                f"{self.vked_attachments.item(row, 1).text()},"

      signed = int(self.vked_signed_2.isChecked())
      publish_date = int(
                        time.mktime(
                           time.strptime(
                              self.vked_publish_date.text(),
                              '%d.%m.%Y %H:%M'
                           )
                        )
                     )
   ...
   def vked_previewW(self):
      ...
      for row in range(0, rowCount):
         if self.vked_attachments.item(row, 0).text() and self.vked_attachments.item(row, 1).text():
            attachments.append(
               [self.vked_attachments.item(row, 0).text(),
                self.vked_attachments.item(row, 1).text(),
               ]
            )
      ...
      for attachment in attachments:
         if attachment[0] == 'photo':
            ...
            message = f'{message}<img src="{url}" width="200px">'
         elif attachment[0] == 'doc':
            url = self.vk.method(
                     'docs.getById',
                     {'docs': attachment[1]}
                  )[0]['preview']['photo']['sizes'][-1]['url']
            message = f'{message}<br>doc: <img src="{url}" width="200px">'

      author = ''
      if self.vked_signed_2.isChecked():
         author_id = self.vk.method(
                        'wall.getById',
                        {'posts': self.vked_postLink.text().split('?')[0].split('wall')[-1]}
                     )[0]['created_by']
         name = self.vk.method('users.get', {'user_ids': author_id})[0]
         author = f"{name['first_name']} {name['last_name']}"
...
#
   ...
   def vkps_download(self):
      try:
         ...
         self.thread.start();
      except (OSError, vk_api.exceptions.ApiError) as err:
         self.statusBar.showMessage(f"{err}", 5000)
         self.vkps_downloadButton.setEnabled(True)
         if err.__class__ is vk_api.exceptions.ApiError:
            print(f"{err}\a")
         else:
            print('\a', end='\r')
  ### Сигналы
